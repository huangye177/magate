...
/**
 * Class AdvancedPolicy (version 1.0)<p>
 ...
 * available. If the gridlet requires more PEs than is available the gridlet is returned
 * as FAILED. In this version we suppose that <b>resource is composed of only ONE machine!</b>
 * Otherwise the simulation may hang.
 * 
 * The corresponding static characteristic and dynamic information of grid resource 
 * is preserved within LRM Storage.
 *
 * @author Manzur Murshed and Rajkumar Buyya
 * @author Anthony Sulistio (re-written this class)
 * @author Dalibor Klusacek (re-named and extended this class)
 * @author Ye HUANG (extend and refacting)
 * @since  GridSim Toolkit 2.2
 */
public class AdvancedPolicy extends AllocPolicy {
    ...
    private String suff = "";
    private MaGateStorage storage = null;
    
    private static Log log = LogFactory.getLog(AdvancedPolicy.class);
    
    /**
     * Allocates a new AdvancedPolicy object
     ...
     * @param suff            suffix for output file with machine usage
     * @param storage 
     *
     ...
     * @post $none
     */
    public AdvancedPolicy(String resourceName, String entityName, MaGateStorage storage) throws Exception {
        ...
        if (gl.getNumPE() > 1) {
            ...
            if(numPE <= freePE && gridletQueueList_.size() == 0){
                ...
                if(gl.getNumPE() > super.totalPE_){
                    ...
                    System.out.println(this.storage.getMaGateIdentity() + ":FAILED....Resource has low number (" + 
                    		super.totalPE_ + ")of PEs to run this gridlet(" + gl.getNumPE() + ") ...");
            ...
            if (resource_.getNumBusyPE() < super.totalPE_ && gridletQueueList_.size() == 0) {
...
//        storage.setResourceJobExecutionLoad(storage.getResourceJobExecutionLoad() + 
//            (storage.getTotalActivePEs().get() / storage.getTotalAvailPEs().get()) * 
//            (GridSim.clock() - storage.getPreEventClock()));
//        storage.setResourceMaxLoad(storage.getResourceMaxLoad() + 
//            1.0 * (GridSim.clock() - storage.getPreEventClock()));
//        storage.setPreEventClock(GridSim.clock());
        
        // ye: reset number of active PE 
        storage.setTotalActivePEs(new AtomicInteger(storage.getTotalActivePEs().get() + 1));
     ...
     * INEXEC and PEs status into busy afterwards. Currently we suppose 1 resource = 1 machine. Otherwise
     * the simulation may hang.
     * @param rgl  a ResGridlet object
    ...
    private boolean allocatePEtoGridlet(ResGridlet rgl, int numPE) {
        // IDENTIFY MACHINE which has enough free PE and add this Gridlet to it.
        Machine myMachine = resource_.getMachineWithFreePE(numPE);
        
        // If the machine does not have enough PEs then ignore the rest
        if (myMachine == null) {
            // to be solved in the future
            System.out.println("Error when allocating PEs.");
            return false;
        }
        
        if(myMachine.getNumFreePE() >= numPE){
            // gets the list of PEs and find one empty PE
            PEList MyPEList = myMachine.getPEList();
            ...
            // set all required PEs to BUSY status
            for(int i=0; i < numPE; i++){
                int freePE = MyPEList.getFreePEID();
                // Allocate this gridlet to Machine and its PEs
                rgl.setMachineAndPEID(myMachine.getMachineID(), freePE);
                // Set allocated PEs to BUSY status
                super.resource_.setStatusPE(PE.BUSY, myMachine.getMachineID(), freePE);
            }
...
//            storage.setPreEventClock(GridSim.clock());
            ...
            // ye: reset number of active PE
            storage.setTotalActivePEs(new AtomicInteger(storage.getTotalActivePEs().get() + numPE));
            ...
            System.out.println("Unwanted error happened...");
            return false;
        }
    }
...
//        storage.setPreEventClock(GridSim.clock());
        ...
        // ye: reset number of active PE
        storage.setTotalActivePEs(new AtomicInteger(storage.getTotalActivePEs().get() - rgl.getNumPE()));
        storage.setNumOfRequestedPEs(new AtomicInteger(storage.getNumOfRequestedPEs().get() - gl.getNumPE()));
